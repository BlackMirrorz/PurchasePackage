#-----------------------------------------------------
# Code Validation
# Configures SwiftLint & SwiftFormat For Our CI Pipeline
# Copyright 2024
...
custom_rules:
  private_set_outlets:
    name: "Private Set Outlets"
    included: ".*\\.swift"
    regex: '@IBOutlet\\s+(private\\(set\\)\\s+)?weak\\s+var\\s+\\w+:\\s+\\w+!'
    message: "IBOutlet should be private or private(set)"
    severity: warning
